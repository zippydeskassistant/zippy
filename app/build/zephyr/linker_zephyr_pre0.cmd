...
usbd_contex_area : ALIGN_WITH_INPUT SUBALIGN(4) { _usbd_contex_list_start = .; KEEP(*(SORT_BY_NAME(._usbd_contex.static.*))); _usbd_contex_list_end = .; } > RAM AT > FLASH
usbd_class_node_area : ALIGN_WITH_INPUT SUBALIGN(4) { _usbd_class_node_list_start = .; KEEP(*(SORT_BY_NAME(._usbd_class_node.static.*))); _usbd_class_node_list_end = .; } > RAM AT > FLASH
    __data_region_end = .;
...
usbd_msc_lun_area : SUBALIGN(4) { _usbd_msc_lun_list_start = .; KEEP(*(SORT_BY_NAME(._usbd_msc_lun.static.*))); _usbd_msc_lun_list_end = .; } > FLASH
.intList :
{
 KEEP(*(.irq_info*))
 KEEP(*(.intList*))
